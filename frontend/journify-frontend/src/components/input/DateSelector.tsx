...
export const DateSelector = ({ date, setDate }: DateSelectorProps) => {
  ...
  return (
    <div>
      <button
        className="inline-flex items-center gap-2 text-[13px] font-medium text-fuchsia-600 bg-fuchsia-200/40 hover:bg-fuchsia-200/70 rounded px-2 py-1 cursor-pointer"
        onClick={() => {
          setOpenDatePicker(true);
        }}
      >
        <MdOutlineDateRange className="text-lg" />
        {format(date || new Date(), "MMM do yyyy", { locale: enUS })}
      </button>

      {openDatePicker && (
        <div className="overflow-y-scroll p-5 bg-fuchsia-50/80 rounded-lg relative pt-9">
          <button
